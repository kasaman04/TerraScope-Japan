...
#### 基本構造（4階層固定）
```
ホーム > 地域別生活情報 > {都道府県名} > {市区町村名}
```

#### 実装ルール
1. **必須配置**: ページヘッダーの直前、`mb-8`のマージンで配置
2. **BASE_URL使用**: 全てのリンクで `import.meta.env.BASE_URL` を使用
3. **階層構造**: 4つの要素で固定（ホーム、ブログ、都道府県、市区町村）
4. **最終項目**: 現在のページ（市区町村名）はリンクなし、太字表示
...
<nav class="mb-8">
  <ol class="flex items-center space-x-2 text-sm text-gray-600">
    ...
    <li><a href={`${import.meta.env.BASE_URL}ja/blog/{県名}/`} class="hover:text-blue-600">{都道府県名}</a></li>
    <li><span class="mx-2">/</span></li>
    <li class="text-gray-900 font-medium">{市区町村名}</li>
  </ol>
</nav>
```

#### リンク先のルール
- **ホーム**: `{BASE_URL}ja/` → トップページ
- **地域別生活情報**: `{BASE_URL}ja/blog/` → ブログ一覧ページ  
- **都道府県**: `{BASE_URL}ja/blog/{県名}/` → 都道府県ページ
- **市区町村**: リンクなし（現在のページ）

#### 注意事項
- 絶対パス（`/ja/`など）は使用禁止 → GitHub Pagesで動作しない
- `import.meta.env.BASE_URL` を必ず使用 → `/TerraScope-Japan/` プレフィックス対応
- セパレーター（`/`）のスタイルは統一する
- ホバー効果は `hover:text-blue-600` で統一
