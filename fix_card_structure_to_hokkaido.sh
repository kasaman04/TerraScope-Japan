...
for file in "${files[@]}"; do
  if [ -f "$file" ]; then
    ...
    sed -i 's/class=\"w-full h-full object-cover \"\" loading=\"lazy\"/class=\"w-full h-full object-cover\" loading=\"lazy\"/g' "$file"
    
    # 3. タイトルのサイズを調整（text-lg を削除）
    sed -i 's/text-lg">/{cityname}</h4>/class=\"font-bold">/{cityname}</h4>/g' "$file"
...
echo "All prefecture card structures unified to Hokkaido style!"